...
/// </para>
/// <para>
/// <b>De quién es esta tabla, y por qué entra por parámetro</b> (PROV-2 §3). Son los precios
/// publicados de UNA casa: la de fábrica, que es la que le factura a la organización. Desde que la
/// columna de proveedor es obligatoria hay que decirlo en cada fila, y el identificador llega
/// desde <c>ModelRatesService</c> —que sí puede preguntárselo al registro— en vez de estar escrito
/// aquí: esto es el dominio, y el dominio no conoce el nombre de ninguna casa.
/// </para>
/// <para>
...
public static class ModelRateSeed
{
    ...
    /// La casa cuya lista de precios publicada es esta tabla: la de fábrica. Va en cada fila.
    /// </param>
    public static ModelRateTable Create(string providerId)
    {
        ...
        void Add(
            ...
            => table.Rates.Add(new ModelRate(
                model, providerId, input, output, cachedInput, cacheWrite,
                EffectiveFrom: VerifiedOn, Note: note));
        ...
        Add("gpt-5-mini", 0.25m, 2.00m, 0.025m);
        Add("gpt-5.3-codex", 1.75m, 14.00m, 0.175m);
        Add("gpt-5.4", 2.50m, 15.00m, 0.25m);
        Add("gpt-5.4-mini", 0.75m, 4.50m, 0.075m);
        Add("gpt-5.4-nano", 0.20m, 1.25m, 0.02m);
        Add("gpt-5.5", 5.00m, 30.00m, 0.50m);
        Add("gpt-5.6-luna", 0.20m, 1.20m, 0.02m, 0.25m);
        Add("gpt-5.6-sol", 2.00m, 10.00m, 0.20m, 2.50m,
            note: "Promocional: 50 % de descuento hasta el 2026-09-03. Después sube.");
        Add("gpt-5.6-terra", 2.00m, 12.00m, 0.20m, 2.50m);
        ...
        Add("claude-haiku-4.5", 1.00m, 5.00m, 0.10m, 1.25m);
        Add("claude-sonnet-4", 3.00m, 15.00m, 0.30m, 3.75m);
        Add("claude-sonnet-4.5", 3.00m, 15.00m, 0.30m, 3.75m);
        Add("claude-sonnet-4.6", 3.00m, 15.00m, 0.30m, 3.75m);
        Add("claude-sonnet-5", 2.00m, 10.00m, 0.20m, 2.50m);
        Add("claude-opus-4.5", 5.00m, 25.00m, 0.50m, 6.25m);
        Add("claude-opus-4.6", 5.00m, 25.00m, 0.50m, 6.25m);
        Add("claude-opus-4.7", 5.00m, 25.00m, 0.50m, 6.25m);
        Add("claude-opus-4.8", 5.00m, 25.00m, 0.50m, 6.25m);
        Add("claude-opus-5", 5.00m, 25.00m, 0.50m, 6.25m);
        Add("claude-fable-5", 10.00m, 50.00m, 1.00m, 12.50m);

        // ---- Copilot · Google, Microsoft, xAI, Moonshot ------------------------------------
        Add("gemini-3.1-pro", 2.00m, 12.00m, 0.20m);
        Add("gemini-3.5-flash", 1.50m, 9.00m, 0.15m);
        Add("gemini-3.6-flash", 0.75m, 3.75m, 0.075m,
            note: "Promocional hasta el 2026-12-31.");
        Add("gemini-3.7-flash", 0.75m, 3.75m, 0.075m,
            note: "Promocional hasta el 2026-12-31.");
        Add("mai-code-1-flash", 0.75m, 4.50m, 0.075m);
        Add("mai-code-1.1-flash", 0.20m, 1.20m, 0.02m);
        Add("grok-4.5", 2.00m, 6.00m, 0.50m);
        Add("grok-4.6", 2.00m, 6.00m, 0.50m);
        Add("kimi-k2.7-code", 0.95m, 4.00m, 0.19m);
        Add("kimi-k3", 3.00m, 15.00m, 0.30m);
        Add("raptor-mini", 0.25m, 2.00m, 0.025m);
    ...
    /// </param>
    /// <param name="Adopted">
    /// Cuántas tarifas ya escritas se han quedado con el proveedor de fábrica porque no tenían
    /// ninguno (PROV-2 §3). Es la migración de los hubs anteriores, y cuenta como algo que
    /// escribir: sin ella la columna obligatoria se quedaría en blanco para siempre.
    /// </param>
    public sealed record SeedFill(
        ModelRateTable Table, IReadOnlyList<ModelRate> Added, int Adopted = 0)
    {
        /// <summary>Hay algo que escribir en el hub: filas nuevas, o filas adoptadas.</summary>
        public bool HasChanges => Added.Count > 0 || Adopted > 0;
    }
    ...
    /// <param name="providerId">La casa de fábrica: de quién son estos precios (PROV-2 §3).</param>
    public static SeedFill Fill(ModelRateTable? existing, string providerId)
    {
        ModelRateTable seed = Create(providerId);
        if (existing is null)
        {
            return new SeedFill(seed, seed.Rates.ToList(), 0);
        }

        // PROV-2 §3 — lo que ya esté escrito SIN proveedor se adopta antes de comparar: es la
        // lista de precios de la casa de fábrica, que era la única que facturaba cuando se sembró.
        // Nadie pierde su tabla y ninguna cifra se mueve; lo que cambia es que ahora se sabe de
        // quién es cada fila.
        int adopted = existing.AdoptProvider(providerId);
        ...
        return new SeedFill(existing, added, adopted);
    }
}
